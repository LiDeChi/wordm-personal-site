...
export function InteractiveHomePage({ lang, onLangChange, themeMode, onThemeToggle }: Props) {
    ...
    const selected = FOUNT_FIELDS.filter(field => ["flipook", "museum-book"].includes(field.key));
    ...
    <main id="home-main">
      ...
      <section id="explorations" className="ih-explorations">
        ...
        <div className="ih-projects">{selected.map((field, index) => <a className="ih-project" key={field.key} href={`${field.href}/?lang=${lang}`}>
          <div className="ih-project-image"><img src={field.coverUrl} alt={field.coverAlt[lang]} loading="lazy"/><span>0{index + 1} ↗</span></div>
          <div className="ih-project-heading"><h3>{field.name}</h3><span>{index === 0 ? (zh ? "空间阅读" : "SPATIAL READING") : (zh ? "可漫游的书" : "WALKABLE BOOKS")}</span></div><p>{index === 0 ? (zh ? "让书页拥有空间，让阅读可以进入、回看与探索。" : "Give pages a spatial dimension, and reading a place to return to.") : (zh ? "把章节展开成展厅，在行走中重新建立内容之间的联系。" : "Unfold chapters into galleries and discover connections by moving through them.")}</p>
        </a>)}</div>
        <a className="ih-minds" href={`/minds?lang=${lang}`}><span>03 / {zh ? "心智探索" : "EXPLORING MINDS"}</span><h3>{zh ? "关于心智的不同解释，放到同一个现场里看。" : "Explore different explanations of mind in one shared space."}</h3><span className="ih-minds-arrow">↗</span></a>
      </section>
